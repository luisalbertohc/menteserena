...
import {
  ...
  VideoCall as AcceptVideoCallIcon,
...
const ConversationPreviewCard = ({
  ...
  const profileUrl = user?.profile_picture
    ? `${config.MENTE_SERENA_API_BASE_URL}/api/profile_picture/${encodeURIComponent(
        ...
        <Grid>
          ...
          <Typography
            className={classnames(classes.description, {
              [classes.iconDescription]: ['file', 'video_call'].includes(messages[messages.length - 1]?.type),
            })}
          >
